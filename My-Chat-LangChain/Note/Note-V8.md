# Stream-Agent V8.0 开发说明文档

> **版本**: V8.0
> **发布日期**: 2025-12-17
> **核心功能**: E2B 云沙箱代码执行集成
> **文档类型**: 详细开发说明与技术参考

---

## 📑 目录

1. [版本概述](#一版本概述)
2. [新增功能详解](#二新增功能详解)
3. [技术架构](#三技术架构)
4. [文件变更清单](#四文件变更清单)
5. [代码实现详解](#五代码实现详解)
6. [配置指南](#六配置指南)
7. [使用教程](#七使用教程)
8. [问题排查指南](#八问题排查指南)
9. [性能优化记录](#九性能优化记录)
10. [API 参考](#十api-参考)
11. [部署指南](#十一部署指南)
12. [开发历程记录](#十二开发历程记录)

---

## 一、版本概述

### 1.1 版本亮点

Stream-Agent V8.0 是一次重大功能升级，引入了 **E2B 云沙箱代码执行能力**，使 AI 助手能够：

| 能力 | V7.0 | V8.0 |
|------|------|------|
| 执行 Python 代码 | ❌ | ✅ |
| 数据分析 (pandas) | ❌ | ✅ |
| 生成可视化图表 | ❌ | ✅ |
| 算法验证与测试 | ❌ | ✅ |
| 数学精确计算 | ❌ | ✅ |
| 文件处理与转换 | ❌ | ✅ |

### 1.2 版本对比

```
V7.0 (90+ 工具)                    V8.0 (96+ 工具)
├── Web 搜索                        ├── Web 搜索
├── 电商数据提取                     ├── 电商数据提取
├── 社交媒体分析                     ├── 社交媒体分析
├── 浏览器自动化                     ├── 浏览器自动化
├── 学术论文搜索                     ├── 学术论文搜索
├── RAG 知识库                      ├── RAG 知识库
└── 结构化输出                      ├── 结构化输出
                                   └── 🆕 E2B 代码执行 (6个工具)
                                       ├── execute_python_code
                                       ├── execute_shell_command
                                       ├── install_python_package
                                       ├── upload_data_to_sandbox
                                       ├── download_file_from_sandbox
                                       └── analyze_csv_data
```

### 1.3 主要成就

- ✅ E2B 云沙箱完整集成
- ✅ 前端图表渲染支持
- ✅ 工具调用效率优化 (6-8次 → 3次)
- ✅ Windows/Linux 跨平台兼容
- ✅ OpenAI 兼容模式支持第三方 LLM
- ✅ 10个关键 Bug 修复

---

## 二、新增功能详解

### 2.1 E2B 代码执行工具

#### 2.1.1 execute_python_code

**功能**: 在安全的云沙箱中执行任意 Python 代码

**特点**:
- 完全隔离的执行环境
- 自动捕获 matplotlib 图表
- 支持多行代码
- 60秒超时保护

**预装库**:
```
pandas, numpy, matplotlib, seaborn, plotly, openpyxl, xlrd, scipy
```

**使用示例**:
```python
# Agent 会自动调用此工具
execute_python_code("""
import pandas as pd
import matplotlib.pyplot as plt

# 创建示例数据
data = {'月份': ['1月', '2月', '3月'], '销售': [100, 150, 200]}
df = pd.DataFrame(data)

# 绑图
plt.figure(figsize=(10, 6))
plt.bar(df['月份'], df['销售'])
plt.title('月度销售趋势')
plt.show()  # 必须调用 plt.show() 来捕获图表
""")
```

#### 2.1.2 upload_data_to_sandbox

**功能**: 将用户上传的文件传输到沙箱环境

**文件路径**:
- 本地临时目录:
  - Windows: `backend/temp_uploads/`
  - Linux: `/tmp/temp_uploads/`
- 沙箱目录: `/home/user/data/`

**返回信息**:
```
✅ 文件上传成功

📁 **文件信息**:
- 文件名: sales.csv
- 大小: 1.5 KB
- 沙箱路径: `/home/user/data/sales.csv`

💡 **使用提示**:
import pandas as pd
df = pd.read_csv("/home/user/data/sales.csv")
```

#### 2.1.3 analyze_csv_data

**功能**: 快速分析 CSV 文件，返回数据概览

**输出内容**:
- 数据维度 (行数 × 列数)
- 列名列表
- 数据类型
- 前5行预览
- 数值列统计摘要
- 缺失值分析
- 分类列统计

#### 2.1.4 execute_shell_command

**功能**: 在沙箱中执行 Shell 命令

**安全限制**:
- 禁止危险命令: `rm -rf /`, `mkfs`, `dd if=`, `:(){`, `fork bomb`
- 30秒超时

**常用场景**:
```bash
ls -la /home/user/data/    # 查看文件列表
pip list                    # 查看已安装包
cat /home/user/data/file   # 查看文件内容
```

#### 2.1.5 install_python_package

**功能**: 安装额外的 Python 包

**预装包检测**: 自动跳过已预装的包

**超时设置**: 120秒

#### 2.1.6 download_file_from_sandbox

**功能**: 从沙箱下载生成的文件

**返回格式**:
- 文本文件: 直接返回内容
- 二进制文件: Base64 编码

### 2.2 LLM 配置增强

V8.0 新增 **OpenAI 兼容模式**，支持第三方 LLM 中转平台。

#### 配置选项

| 模式 | 配置项 | 说明 |
|------|--------|------|
| Google Gemini | `LLM_PROVIDER=google` | 使用 Google 官方 API |
|  | `GOOGLE_MODEL` | 模型名称，默认 `gemini-2.0-flash-lite` |
| OpenAI 兼容 | `LLM_PROVIDER=openai_compatible` | 使用第三方中转 |
|  | `OPENAI_BASE_URL` | API 端点 URL |
|  | `OPENAI_API_KEY` | API 密钥 |
|  | `OPENAI_MODEL` | 模型名称 |

#### 前端配置界面

```
🧠 LLM Configuration
├── ○ Google Gemini (Official)
│   └── Model Name: [gemini-2.0-flash-lite]
└── ○ OpenAI Compatible (Proxy)
    ├── Base URL: [https://api.openai.com/v1]
    ├── API Key: [********]
    └── Model Name: [gpt-4o-mini]
```

### 2.3 前端图表渲染

V8.0 实现了完整的图表渲染管道：

```
E2B 执行代码
    ↓
matplotlib plt.show()
    ↓
E2B 自动捕获 PNG
    ↓
Base64 编码 [IMAGE_BASE64:xxx]
    ↓
后端保留完整数据 (不截断)
    ↓
前端解码并渲染 st.image()
```

**兼容性处理**:
```python
try:
    container.image(image_bytes, use_container_width=True)
except TypeError:
    container.image(image_bytes, use_column_width=True)
```

---

## 三、技术架构

### 3.1 整体架构图

```
┌─────────────────────────────────────────────────────────────────────┐
│                     Stream-Agent V8.0 Architecture                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │                    Frontend (Streamlit)                       │   │
│  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────────────────┐ │   │
│  │  │ Chat UI     │ │ File Upload │ │ LLM Config Panel        │ │   │
│  │  │             │ │ (PDF,CSV,   │ │ ├─ Google Gemini        │ │   │
│  │  │ - Messages  │ │  Excel,JSON │ │ └─ OpenAI Compatible    │ │   │
│  │  │ - Charts    │ │  TXT,PY)    │ │                         │ │   │
│  │  │ - Tools     │ │             │ │ API Keys Input          │ │   │
│  │  └─────────────┘ └─────────────┘ └─────────────────────────┘ │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                              │ SSE Stream                            │
│                              ▼                                       │
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │                    Backend (FastAPI)                          │   │
│  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────────────────┐ │   │
│  │  │ /chat/stream│ │/upload_file │ │ CORS Middleware         │ │   │
│  │  │ POST        │ │ POST        │ │ allow_origins=["*"]     │ │   │
│  │  └─────────────┘ └─────────────┘ └─────────────────────────┘ │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                              │                                       │
│                              ▼                                       │
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │                 Agent Service (LangGraph)                     │   │
│  │  ┌─────────────────────────────────────────────────────────┐ │   │
│  │  │                    ReAct Agent                           │ │   │
│  │  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │ │   │
│  │  │  │ LLM          │  │ Tools (96+)  │  │ Checkpointer │  │ │   │
│  │  │  │ - Gemini     │  │ - MCP Tools  │  │ - SQLite     │  │ │   │
│  │  │  │ - OpenAI     │  │ - Custom     │  │              │  │ │   │
│  │  │  │   Compatible │  │ - E2B Tools  │  │              │  │ │   │
│  │  │  └──────────────┘  └──────────────┘  └──────────────┘  │ │   │
│  │  └─────────────────────────────────────────────────────────┘ │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                              │                                       │
│          ┌──────────────────┼──────────────────┐                    │
│          ▼                  ▼                  ▼                    │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐      │
│  │ MCP Servers  │  │ RAG System   │  │    E2B Cloud         │      │
│  │ - BrightData │  │ - ChromaDB   │  │    Sandbox           │      │
│  │ - PaperSearch│  │ - Embeddings │  │  ┌────────────────┐  │      │
│  └──────────────┘  └──────────────┘  │  │ Python Runtime │  │      │
│                                      │  │ - pandas       │  │      │
│                                      │  │ - matplotlib   │  │      │
│                                      │  │ - numpy        │  │      │
│                                      │  └────────────────┘  │      │
│                                      │  ┌────────────────┐  │      │
│                                      │  │ File System    │  │      │
│                                      │  │ /home/user/    │  │      │
│                                      │  └────────────────┘  │      │
│                                      └──────────────────────┘      │
└─────────────────────────────────────────────────────────────────────┘
```

### 3.2 E2B 沙箱生命周期

```
┌─────────────────────────────────────────────────────────────────┐
│                    E2B Sandbox Lifecycle                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  用户首次请求代码执行                                              │
│         │                                                        │
│         ▼                                                        │
│  ┌─────────────────┐                                            │
│  │ get_sandbox()   │                                            │
│  │ 检查沙箱是否存在  │                                            │
│  └────────┬────────┘                                            │
│           │                                                      │
│     ┌─────┴─────┐                                               │
│     │           │                                               │
│     ▼           ▼                                               │
│  不存在      存在                                                │
│     │           │                                               │
│     ▼           ▼                                               │
│ ┌──────────┐  ┌──────────────┐                                  │
│ │创建新沙箱 │  │ ping 检测    │                                  │
│ │ timeout: │  │ 执行简单命令  │                                  │
│ │ 10 min   │  └──────┬───────┘                                  │
│ └────┬─────┘         │                                          │
│      │         ┌─────┴─────┐                                    │
│      ▼         │           │                                    │
│ ┌──────────┐  成功        失败                                   │
│ │ 预装依赖  │   │           │                                    │
│ │ pandas   │   │           ▼                                    │
│ │ numpy    │   │    ┌──────────┐                                │
│ │ matplotlib│  │    │ 重建沙箱  │                                │
│ │ ...      │   │    └────┬─────┘                                │
│ └────┬─────┘   │         │                                      │
│      │         │         │                                      │
│      └─────────┴─────────┘                                      │
│                │                                                 │
│                ▼                                                 │
│         ┌──────────┐                                            │
│         │ 返回沙箱  │                                            │
│         │ 实例     │                                            │
│         └──────────┘                                            │
│                │                                                 │
│                ▼                                                 │
│    ┌───────────────────────┐                                    │
│    │ 执行代码/命令         │                                    │
│    │ sandbox.run_code()    │                                    │
│    └───────────────────────┘                                    │
│                │                                                 │
│         ┌──────┴──────┐                                         │
│         │             │                                         │
│    正常输出      图表输出                                         │
│         │             │                                         │
│         ▼             ▼                                         │
│    stdout/       result.png                                     │
│    stderr        → Base64                                       │
│         │             │                                         │
│         └──────┬──────┘                                         │
│                ▼                                                 │
│         ┌──────────┐                                            │
│         │ 返回结果  │                                            │
│         └──────────┘                                            │
│                                                                  │
│  [10分钟无操作后自动销毁]                                          │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### 3.3 数据流详解

#### 3.3.1 图表生成数据流

```
用户: "画一个正弦波图表"
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ Agent 识别意图 → 调用 execute_python_code                    │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ E2B 沙箱执行代码:                                            │
│ import numpy as np                                          │
│ import matplotlib.pyplot as plt                             │
│ x = np.linspace(0, 2*np.pi, 100)                           │
│ y = np.sin(x)                                               │
│ plt.plot(x, y)                                              │
│ plt.title('正弦波')                                          │
│ plt.show()  ← 触发图表捕获                                   │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ E2B 返回 execution.results[0].png (Base64 字符串)            │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ 工具返回:                                                    │
│ "🖼️ **图表已生成并显示在上方**                               │
│  [IMAGE_BASE64:iVBORw0KGgoAAAANSUhEUgAAA...]"              │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ agent_service.py on_tool_end 处理:                          │
│ - 检测 [IMAGE_BASE64:...] 标记                              │
│ - 保留完整 Base64 数据 (不截断)                               │
│ - JSON 编码后 Base64 编码整体数据                             │
│ - SSE 发送: event: tool_end\ndata: {encoded}               │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ 前端 app.py 接收:                                           │
│ - Base64 解码获取 JSON                                      │
│ - 提取 tool_output                                          │
│ - 正则匹配 [IMAGE_BASE64:(...)]                             │
│ - base64.b64decode() 获取图片字节                            │
│ - st.image() 渲染图表                                       │
└─────────────────────────────────────────────────────────────┘
```

#### 3.3.2 CSV 分析数据流

```
用户上传 sales.csv
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ /upload_file API:                                           │
│ - 保存到 temp_uploads/sales.csv                             │
│ - 调用 ingest_file() 加入 RAG 知识库                         │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
用户: "分析上传的文件并画销售趋势图"
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ 工具调用链 (优化后仅3次):                                     │
│                                                              │
│ 1. upload_data_to_sandbox("sales.csv")                      │
│    → 文件传输到沙箱 /home/user/data/sales.csv               │
│                                                              │
│ 2. execute_python_code(读取并显示列名)                       │
│    df = pd.read_csv(path)                                   │
│    print(df.columns.tolist())                               │
│    print(df.head())                                         │
│    → 输出: ['month', 'sales', 'profit']                     │
│                                                              │
│ 3. execute_python_code(根据实际列名画图)                     │
│    plt.plot(df['month'], df['sales'])                       │
│    plt.title('销售趋势图')                                   │
│    plt.show()                                                │
│    → 返回图表 Base64                                         │
└─────────────────────────────────────────────────────────────┘
```

---

## 四、文件变更清单

### 4.1 新增文件

| 文件路径 | 说明 | 代码行数 |
|---------|------|---------|
| `backend/tools/e2b_tools.py` | E2B 工具模块 | ~500 行 |
| `backend/test_sales.csv` | 测试数据文件 | 7 行 |

### 4.2 修改文件

| 文件路径 | 修改类型 | 主要变更 |
|---------|---------|---------|
| `backend/agent_service.py` | 重大修改 | E2B工具集成、OpenAI兼容、Prompt优化 |
| `backend/main.py` | 小修改 | Windows路径兼容 |
| `backend/requirements.txt` | 新增依赖 | e2b, e2b-code-interpreter, langchain-openai |
| `frontend/app.py` | 重大修改 | LLM配置UI、图表渲染、Base64过滤 |
| `Plan-V8.md` | 文档更新 | 开发进度、问题记录 |

### 4.3 详细变更内容

#### backend/agent_service.py

```diff
+ from langchain_openai import ChatOpenAI

# 工具导入变更
- from tools.e2b_tools import (
-     execute_python_code,
-     execute_shell_command,
-     install_python_package,
-     upload_data_to_sandbox,
-     download_file_from_sandbox,
-     create_visualization,
-     analyze_csv_data,
-     generate_chart_from_data,
-     close_sandbox
- )
+ from tools.e2b_tools import (
+     execute_python_code,
+     execute_shell_command,
+     install_python_package,
+     upload_data_to_sandbox,
+     download_file_from_sandbox,
+     analyze_csv_data,
+     close_sandbox
+ )

# System Prompt 优化
+ **重要提示 - 必须遵守**:
+ 1. **数据分析+图表任务的标准流程(严格按顺序执行)**:
+    - 第一步: `upload_data_to_sandbox(文件名)` 上传文件
+    - 第二步: **先用一段代码读取数据并查看列名**
+    - 第三步: **根据实际列名**编写画图代码
+    - **严禁猜测列名**！必须先读取确认

# LLM 配置 - 新增 OpenAI 兼容模式
+ if llm_provider == "openai_compatible":
+     llm = ChatOpenAI(
+         model=openai_model,
+         base_url=openai_base_url,
+         api_key=openai_api_key,
+         temperature=0
+     )

# 工具输出处理 - 保留完整图片
+ if "[IMAGE_BASE64:" in output:
+     # 提取并保留图片数据，只截断文本部分
+     image_pattern = r'\[IMAGE_BASE64:[A-Za-z0-9+/=]+\]'
+     images = re.findall(image_pattern, output)
+     # ... 重组输出保留完整图片
```

#### backend/tools/e2b_tools.py

```diff
# 沙箱超时增加到10分钟
- timeout=300,  # 5分钟超时
+ timeout=600,  # 10分钟超时

# 添加沙箱有效性检测
+ try:
+     await _sandbox.run_code("print('ping')", timeout=5)
+     return _sandbox
+ except Exception as e:
+     print(f"⚠️ [E2B] 沙箱已失效，正在重新创建...")
+     _sandbox = await _create_new_sandbox()

# Windows 路径兼容
+ if platform.system() == "Windows":
+     temp_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "temp_uploads")
+ else:
+     temp_dir = "/tmp/temp_uploads"
```

#### frontend/app.py

```diff
# 版本号更新
- st.title("🤖 Stream-Agent v7.0")
+ st.title("🤖 Stream-Agent v8.0")

# 新增 LLM 配置面板
+ with st.expander("🧠 LLM Configuration", expanded=True):
+     llm_provider = st.radio(
+         "Select LLM Provider",
+         options=["google", "openai_compatible"],
+         ...
+     )

# 图表渲染兼容性
+ try:
+     container.image(image_bytes, use_container_width=True)
+ except TypeError:
+     container.image(image_bytes, use_column_width=True)

# 过滤流式文本中的 Base64
+ if "[IMAGE_BASE64:" in text_content:
+     text_content = re.sub(r'\[IMAGE_BASE64:[A-Za-z0-9+/=]+\]',
+                           '[图表已在上方工具结果中显示]', text_content)
```

#### backend/requirements.txt

```diff
+ langchain-openai>=0.3.0

  # E2B Code Interpreter (Cloud Sandbox)
  e2b>=1.0.0
  e2b-code-interpreter>=1.0.0
```

---

## 五、代码实现详解

### 5.1 E2B 沙箱管理

#### 单例模式实现

```python
# backend/tools/e2b_tools.py

_sandbox: Optional[AsyncSandbox] = None
_sandbox_lock: Optional[asyncio.Lock] = None

def _get_lock() -> asyncio.Lock:
    """懒加载 Lock，避免模块加载时事件循环问题"""
    global _sandbox_lock
    if _sandbox_lock is None:
        _sandbox_lock = asyncio.Lock()
    return _sandbox_lock

async def get_sandbox() -> AsyncSandbox:
    """获取或创建 E2B Sandbox 单例"""
    global _sandbox

    async with _get_lock():
        if _sandbox is None:
            _sandbox = await _create_new_sandbox()
            return _sandbox

        # 有效性检测
        try:
            await _sandbox.run_code("print('ping')", timeout=5)
            return _sandbox
        except Exception:
            _sandbox = await _create_new_sandbox()
            return _sandbox
```

**设计考量**:
1. **单例模式**: 避免频繁创建沙箱，节省时间和成本
2. **懒加载 Lock**: 解决 `asyncio.Lock()` 在模块加载时的事件循环问题
3. **有效性检测**: 沙箱超时后自动重建

#### 沙箱创建流程

```python
async def _create_new_sandbox() -> AsyncSandbox:
    api_key = os.environ.get("E2B_API_KEY")
    if not api_key:
        raise ValueError("E2B_API_KEY 环境变量未设置")

    sandbox = await AsyncSandbox.create(
        api_key=api_key,
        timeout=600,  # 10分钟超时
    )

    # 预装依赖
    await sandbox.run_code("""
import subprocess
subprocess.run(['pip', 'install', '-q',
    'pandas', 'numpy', 'matplotlib', 'seaborn',
    'plotly', 'openpyxl', 'xlrd', 'scipy'],
    capture_output=True)
""", timeout=120)

    return sandbox
```

### 5.2 代码执行工具

```python
@tool
async def execute_python_code(code: str) -> str:
    """在安全的云沙箱中执行 Python 代码"""
    sandbox = await get_sandbox()
    execution = await sandbox.run_code(code, timeout=60)

    output_parts = []

    # 处理标准输出
    if execution.logs and execution.logs.stdout:
        stdout_content = '\n'.join(execution.logs.stdout)
        output_parts.append(f"📤 **输出**:\n```\n{stdout_content}\n```")

    # 处理图片结果
    if execution.results:
        for result in execution.results:
            if hasattr(result, 'png') and result.png:
                output_parts.append(
                    f"🖼️ **图表已生成并显示在上方**\n[IMAGE_BASE64:{result.png}]"
                )

    # 处理错误
    if execution.error:
        output_parts.append(f"❌ **执行错误**:\n{execution.error}")

    return "\n\n".join(output_parts)
```

**关键点**:
1. **E2B Code Interpreter API**: 使用 `run_code()` 而非 `commands.run()`
2. **图表自动捕获**: `plt.show()` 后图片在 `execution.results[0].png`
3. **错误处理**: 完整捕获并格式化错误信息

### 5.3 图表渲染管道

#### 后端处理 (agent_service.py)

```python
elif kind == "on_tool_end":
    output = str(event["data"].get("output", ""))

    # 检测图片数据
    if "[IMAGE_BASE64:" in output:
        image_pattern = r'\[IMAGE_BASE64:[A-Za-z0-9+/=]+\]'
        images = re.findall(image_pattern, output)
        text_parts = re.split(image_pattern, output)

        # 只截断文本，保留完整图片
        truncated_text_parts = [
            (part[:500] + '...' if len(part) > 500 else part)
            for part in text_parts
        ]

        # 重组输出
        safe_output = ""
        for i, text_part in enumerate(truncated_text_parts):
            safe_output += text_part
            if i < len(images):
                safe_output += images[i]
    else:
        safe_output = (output[:1000] + '...') if len(output) > 1000 else output
```

#### 前端渲染 (app.py)

```python
def render_tool_output(output_str, container):
    image_pattern = r'\[IMAGE_BASE64:([A-Za-z0-9+/=]+)\]'
    matches = list(re.finditer(image_pattern, output_str))

    if matches:
        for match in matches:
            image_b64 = match.group(1)
            image_bytes = base64.b64decode(image_b64)

            # Streamlit 版本兼容
            try:
                container.image(image_bytes, caption="📊 Generated Chart",
                              use_container_width=True)
            except TypeError:
                container.image(image_bytes, caption="📊 Generated Chart",
                              use_column_width=True)
```

### 5.4 LLM 配置切换

```python
# backend/agent_service.py

llm_provider = api_keys.get("LLM_PROVIDER", "google")

if llm_provider == "openai_compatible":
    llm = ChatOpenAI(
        model=api_keys.get("OPENAI_MODEL", "gpt-4o-mini"),
        base_url=api_keys.get("OPENAI_BASE_URL"),
        api_key=api_keys.get("OPENAI_API_KEY"),
        temperature=0
    )
else:
    llm = ChatGoogleGenerativeAI(
        model=api_keys.get("GOOGLE_MODEL", "gemini-2.0-flash-lite"),
        google_api_key=os.environ["GOOGLE_API_KEY"],
        temperature=0
    )
```

---

## 六、配置指南

### 6.1 环境变量配置

创建 `backend/.env` 文件：

```bash
# ============================================================
# LLM 配置 (二选一)
# ============================================================

# 方式1: Google Gemini (默认)
GOOGLE_API_KEY=your_google_api_key
GOOGLE_MODEL=gemini-2.0-flash-lite

# 方式2: OpenAI 兼容模式 (可选)
# LLM_PROVIDER=openai_compatible
# OPENAI_BASE_URL=https://api.openrouter.ai/api/v1
# OPENAI_API_KEY=your_api_key
# OPENAI_MODEL=google/gemini-2.0-flash-exp:free

# ============================================================
# MCP 工具 API Keys
# ============================================================

BRIGHT_DATA_API_KEY=your_bright_data_key
PAPER_SEARCH_API_KEY=your_paper_search_key

# ============================================================
# E2B 代码执行 (必须)
# ============================================================

E2B_API_KEY=your_e2b_api_key
```

### 6.2 获取 API Key

| 服务 | 获取地址 | 免费额度 |
|------|---------|---------|
| E2B | https://e2b.dev/dashboard | Hobby 计划免费 |
| Google AI | https://aistudio.google.com/apikey | 免费 |
| BrightData | https://brightdata.com | 试用 |
| Paper Search | Smithery.ai | 免费 |

### 6.3 前端配置

在 Streamlit 侧边栏可动态配置：

1. **LLM Configuration**: 选择 Google 或 OpenAI Compatible
2. **Tool API Keys**: 输入各工具的 API Key
3. **File Upload**: 上传数据文件

---

## 七、使用教程

### 7.1 基础代码执行

**用户输入**:
```
帮我计算 1 到 100 的和
```

**Agent 执行**:
```python
execute_python_code("""
total = sum(range(1, 101))
print(f"1 到 100 的和是: {total}")
""")
```

**输出**:
```
📤 输出:
1 到 100 的和是: 5050

✅ 代码执行成功
```

### 7.2 数学图表生成

**用户输入**:
```
画一个正弦波和余弦波的对比图
```

**Agent 执行**:
```python
execute_python_code("""
import numpy as np
import matplotlib.pyplot as plt

x = np.linspace(0, 2*np.pi, 100)

plt.figure(figsize=(10, 6))
plt.plot(x, np.sin(x), label='sin(x)', color='blue')
plt.plot(x, np.cos(x), label='cos(x)', color='red')
plt.title('正弦波与余弦波对比')
plt.xlabel('x')
plt.ylabel('y')
plt.legend()
plt.grid(True)
plt.show()
""")
```

**输出**: 显示对比图表

### 7.3 CSV 数据分析

**步骤 1**: 上传 CSV 文件 (通过侧边栏)

**步骤 2**: 发送消息
```
分析上传的 sales.csv 文件，找出销售最好的月份，并画一个趋势图
```

**Agent 执行流程**:
```
1. upload_data_to_sandbox("sales.csv")
   → 文件上传到沙箱

2. execute_python_code(读取列名)
   → 输出: ['month', 'sales', 'profit']

3. execute_python_code(分析并画图)
   → 输出分析结果 + 趋势图
```

### 7.4 算法验证

**用户输入**:
```
写一个快速排序算法并用测试用例验证
```

**Agent 执行**:
```python
execute_python_code("""
def quicksort(arr):
    if len(arr) <= 1:
        return arr
    pivot = arr[len(arr) // 2]
    left = [x for x in arr if x < pivot]
    middle = [x for x in arr if x == pivot]
    right = [x for x in arr if x > pivot]
    return quicksort(left) + middle + quicksort(right)

# 测试用例
test_cases = [
    [3, 6, 8, 10, 1, 2, 1],
    [],
    [1],
    [5, 4, 3, 2, 1],
    [1, 2, 3, 4, 5],
]

print("快速排序测试结果:")
for i, arr in enumerate(test_cases):
    result = quicksort(arr.copy())
    expected = sorted(arr)
    status = "✅" if result == expected else "❌"
    print(f"{status} 测试 {i+1}: {arr} → {result}")
""")
```

### 7.5 安装额外包

**用户输入**:
```
帮我安装 requests 库并测试 HTTP 请求
```

**Agent 执行**:
```
1. install_python_package("requests")
   → ✅ 成功安装 requests

2. execute_python_code("""
   import requests
   response = requests.get('https://httpbin.org/get')
   print(f"状态码: {response.status_code}")
   print(f"响应: {response.json()}")
   """)
```

---

## 八、问题排查指南

### 8.1 常见错误及解决方案

#### 错误 1: E2B_API_KEY 未设置

**错误信息**:
```
ValueError: E2B_API_KEY 环境变量未设置。请在 .env 文件中配置 E2B_API_KEY。
```

**解决方案**:
1. 访问 https://e2b.dev/dashboard 获取 API Key
2. 在 `backend/.env` 添加 `E2B_API_KEY=your_key`
3. 或在前端侧边栏输入 API Key

#### 错误 2: 沙箱超时 502 错误

**错误信息**:
```
The sandbox was not found, code: 502
```

**原因**: 沙箱超时被自动销毁

**解决方案**: V8.0 已自动处理，沙箱会自动重建

#### 错误 3: 图表不显示

**可能原因**:
1. 代码中没有 `plt.show()`
2. Base64 数据被截断
3. 前端渲染错误

**排查步骤**:
1. 确认代码以 `plt.show()` 结尾
2. 检查后端日志中 `[IMAGE_BASE64:` 是否完整
3. 检查浏览器控制台是否有错误

#### 错误 4: 文件上传后找不到

**错误信息**:
```
❌ 找不到文件: data.csv。临时上传目录为空，请先上传文件。
```

**解决方案**:
1. 确认文件已通过前端上传
2. 检查 `temp_uploads/` 目录是否存在文件
3. 使用准确的文件名 (区分大小写)

#### 错误 5: Streamlit expander 嵌套错误

**错误信息**:
```
StreamlitAPIException: Expanders may not be nested inside other expanders.
```

**原因**: `st.status()` 内部不能使用 `st.expander()`

**解决方案**: V8.0 已修复，直接在 status_container 中渲染

### 8.2 调试技巧

#### 查看后端日志

```bash
# 启动后端时查看详细日志
cd backend
python -m uvicorn main:app --reload --log-level debug
```

#### 检查沙箱状态

在前端发送:
```
帮我执行: import sys; print(sys.version); import pandas; print(pandas.__version__)
```

#### 查看可用文件

```
帮我执行 Shell 命令: ls -la /home/user/data/
```

---

## 九、性能优化记录

### 9.1 工具调用优化

| 优化项 | 优化前 | 优化后 | 改进 |
|--------|--------|--------|------|
| CSV分析+画图 | 6-8次工具调用 | 3次 | 减少 50%+ |
| 数学图表 | 3-4次 | 1-2次 | 减少 50% |
| 列名猜测错误 | 频繁 | 几乎无 | 显著改善 |

### 9.2 优化措施

1. **简化工具集**: 从 8 个工具减少到 6 个
   - 移除 `create_visualization`
   - 移除 `generate_chart_from_data`
   - 统一使用 `execute_python_code`

2. **优化 System Prompt**:
   ```
   **严禁猜测列名**！必须先读取确认
   第一步: upload_data_to_sandbox
   第二步: 读取列名
   第三步: 根据实际列名画图
   ```

3. **沙箱复用**: 单例模式避免重复创建

### 9.3 性能指标

| 操作 | 耗时 |
|------|------|
| 首次创建沙箱 | 3-5秒 |
| 预装依赖 | 30-60秒 (仅首次) |
| 简单代码执行 | 1-2秒 |
| 数据分析 | 2-5秒 |
| 图表生成 | 3-8秒 |
| 沙箱 ping 检测 | <1秒 |

---

## 十、API 参考

### 10.1 后端 API

#### POST /chat/stream

流式聊天接口，返回 SSE 事件流。

**请求体**:
```json
{
  "message": "用户消息",
  "thread_id": "会话ID",
  "api_keys": {
    "LLM_PROVIDER": "google",
    "GOOGLE_MODEL": "gemini-2.0-flash-lite",
    "E2B_API_KEY": "xxx",
    "BRIGHT_DATA_API_KEY": "xxx"
  }
}
```

**SSE 事件类型**:
| 事件 | 数据格式 | 说明 |
|------|---------|------|
| `text` | Base64 编码文本 | AI 回复内容 |
| `tool_start` | Base64 编码工具名 | 工具开始执行 |
| `tool_end` | Base64 编码 JSON | 工具执行结果 |
| `done` | "complete" | 流结束 |

#### POST /upload_file

上传文件接口。

**请求**: multipart/form-data
**响应**:
```json
{
  "filename": "data.csv",
  "message": "File data.csv uploaded and learned successfully!"
}
```

### 10.2 E2B 工具 API

#### execute_python_code(code: str) -> str

执行 Python 代码。

**参数**:
- `code`: Python 代码字符串

**返回**: 执行结果，可能包含 `[IMAGE_BASE64:xxx]` 图片标记

#### upload_data_to_sandbox(filename: str) -> str

上传文件到沙箱。

**参数**:
- `filename`: 本地临时目录中的文件名

**返回**: 上传结果和沙箱路径

#### analyze_csv_data(filename: str, analysis_request: str) -> str

分析 CSV 文件。

**参数**:
- `filename`: 文件路径或文件名
- `analysis_request`: 分析需求描述

**返回**: 数据概览和统计信息

#### execute_shell_command(command: str) -> str

执行 Shell 命令。

**参数**:
- `command`: Shell 命令

**返回**: 命令输出

#### install_python_package(package_name: str) -> str

安装 Python 包。

**参数**:
- `package_name`: 包名，支持版本指定

**返回**: 安装结果

#### download_file_from_sandbox(sandbox_path: str) -> str

下载沙箱文件。

**参数**:
- `sandbox_path`: 沙箱中的文件路径

**返回**: 文件内容或 Base64 编码

---

## 十一、部署指南

### 11.1 本地开发

```bash
# 1. 克隆项目
git clone <repo_url>
cd My-Chat-LangChain

# 2. 安装后端依赖
cd backend
pip install -r requirements.txt

# 3. 配置环境变量
cp .env.example .env
# 编辑 .env 填入 API Keys

# 4. 启动后端
uvicorn main:app --reload --port 8000

# 5. 启动前端 (新终端)
cd ../frontend
streamlit run app.py --server.port 8501
```

### 11.2 Render 部署

#### 后端服务配置

```yaml
# render.yaml
services:
  - type: web
    name: stream-agent-backend
    env: python
    buildCommand: pip install -r requirements.txt
    startCommand: uvicorn main:app --host 0.0.0.0 --port $PORT
    envVars:
      - key: GOOGLE_API_KEY
        sync: false
      - key: E2B_API_KEY
        sync: false
      - key: BRIGHT_DATA_API_KEY
        sync: false
```

#### 前端服务配置

```yaml
  - type: web
    name: stream-agent-frontend
    env: python
    buildCommand: pip install streamlit requests
    startCommand: streamlit run app.py --server.port $PORT --server.address 0.0.0.0
    envVars:
      - key: BACKEND_URL
        value: https://stream-agent-backend.onrender.com
```

### 11.3 环境变量清单

| 变量名 | 必须 | 说明 |
|--------|------|------|
| `GOOGLE_API_KEY` | 是* | Google AI API Key |
| `E2B_API_KEY` | 是 | E2B 沙箱 API Key |
| `BRIGHT_DATA_API_KEY` | 否 | BrightData MCP Key |
| `PAPER_SEARCH_API_KEY` | 否 | Paper Search MCP Key |
| `LLM_PROVIDER` | 否 | 默认 "google" |
| `OPENAI_BASE_URL` | 否 | OpenAI 兼容模式 URL |
| `OPENAI_API_KEY` | 否 | OpenAI 兼容模式 Key |
| `OPENAI_MODEL` | 否 | OpenAI 兼容模式模型 |

*如果使用 OpenAI 兼容模式，可以不设置

---

## 十二、开发历程记录

### 12.1 时间线

| 时间 | 事件 |
|------|------|
| 2025-12-17 上午 | 开始 E2B 集成开发 |
| 2025-12-17 上午 | 完成基础工具实现 |
| 2025-12-17 下午 | 遇到并解决 SSE 代理问题 |
| 2025-12-17 下午 | 修复 E2B API 兼容性问题 |
| 2025-12-17 下午 | 修复图表不显示问题 |
| 2025-12-17 下午 | 修复 Streamlit 嵌套错误 |
| 2025-12-17 下午 | 修复沙箱超时问题 |
| 2025-12-17 下午 | 优化工具调用效率 |
| 2025-12-17 晚上 | 完成所有测试，V8.0 发布 |

### 12.2 解决的技术问题

| # | 问题 | 原因 | 解决方案 |
|---|------|------|----------|
| 1 | 前端卡在 "Thinking..." | 代理节点问题 | 切换节点 |
| 2 | `'str' object has no attribute 'line'` | E2B API 变更 | 修改 stdout 处理 |
| 3 | `asyncio.Lock()` 错误 | 模块加载时创建 | 懒加载 |
| 4 | 图表不显示 | Base64 被截断 | 保留完整数据 |
| 5 | expander 嵌套错误 | Streamlit 限制 | 直接渲染 |
| 6 | use_container_width 错误 | 版本差异 | try/except |
| 7 | 沙箱 502 错误 | 超时失效 | 自动重建 |
| 8 | 文件路径错误 | Windows 路径 | platform 判断 |
| 9 | LLM 输出 Base64 | 复述工具输出 | 过滤 + Prompt |
| 10 | 工具调用混乱 | 工具过多 | 简化 + Prompt |

### 12.3 经验教训

1. **E2B SDK 版本**: 使用 `>=1.0.0` 而非固定版本
2. **异步锁**: 不要在模块级别创建 `asyncio.Lock()`
3. **图片传输**: Base64 数据必须完整保留
4. **Streamlit 兼容**: 使用 try/except 处理 API 差异
5. **工具设计**: 越少越好，减少 LLM 选择困难
6. **Prompt 工程**: 明确的流程指导比多个工具更有效

---

## 附录

### A. 测试数据文件

`backend/test_sales.csv`:
```csv
month,sales,profit
January,1000,200
February,1200,250
March,1500,300
April,1300,280
May,1800,400
June,2000,450
```

### B. 相关文档

- [Plan-V8.md](./Plan-V8.md) - 开发计划文档
- [README.md](./README.md) - 项目说明
- [E2B 官方文档](https://e2b.dev/docs)
- [LangChain 文档](https://python.langchain.com/)

### C. 联系与反馈

如有问题或建议，请提交 Issue 或 PR。

---

> **文档版本**: 1.0
> **最后更新**: 2025-12-17
> **作者**: Claude Code Assistant
