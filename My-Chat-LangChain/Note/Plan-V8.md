...
### 12.1 问题 1: 工具调用思维链内容消失

**现象**: 在同一会话中发送新的提问后，之前回复中的工具调用思维链内容 (`st.status` 区域) 会消失。

**原因分析**:
- Streamlit 的 `st.status` 组件在页面重新渲染时不会保留状态
- 当前只保存了最终文本回复到 `st.session_state.messages`，未保存工具调用过程

**改进方案**:
1. **方案 A (简单)**: 将工具调用摘要追加到消息历史中
   - 在 `stream_generator` 完成后，将工具调用记录格式化为文本追加到回复
   - 优点: 实现简单
   - 缺点: 丢失折叠/展开交互

2. **方案 B (完整)**: 扩展消息数据结构
   - 修改 `st.session_state.messages` 结构，添加 `tool_calls` 字段
   - 在显示历史消息时重新渲染工具调用区域
   - 优点: 完整保留交互体验
   - 缺点: 实现复杂度较高

**建议**: 采用方案 A，在回复末尾添加工具调用摘要（如 "🛠️ 使用了 3 个工具: search_engine, scrape_as_markdown ×2"）

---

### 12.2 问题 2: 工具链输出和正文输出杂乱

**现象**:
1. 工具输出原始内容过长，包含大量 URL 和 JSON 数据
2. 抓取的网页内容直接显示，格式混乱
3. 最终回复中图片和文本混排不清晰

**原因分析**:
1. `tool_end` 事件的 output 未做格式化处理
2. `scrape_as_markdown` 返回的原始 Markdown 直接显示
3. 正文中的外部图片 URL 被 Streamlit 自动渲染

**改进方案**:

#### 2.1 工具输出格式化
```python
# 在 render_tool_output 中添加智能截断和格式化
def format_tool_output(tool_name, output):
    if tool_name == "search_engine":
        # 只显示搜索结果数量和前 3 个标题
        return f"找到 X 条结果: 1. xxx 2. xxx 3. xxx ..."
    elif tool_name == "scrape_as_markdown":
        # 显示抓取状态和字数
        return f"成功抓取网页 (约 {len(output)} 字符)"
    else:
        # 默认截断显示
...
#### 2.2 正文输出清理
- 过滤重复的工具调用结果（已在工具区域显示）
- 将外部图片 URL 转为可点击链接而非自动渲染
- 添加 CSS 样式区分引用内容和 AI 分析内容

#### 2.3 UI 布局优化
- 工具调用区域默认折叠，只显示工具名称列表
- 正文区域使用引用块 (`>`) 显示来源内容
- AI 分析结论使用醒目样式
...
- [x] 前端自动加载 `backend/.env` 环境变量 (2025-12-18)
- [x] OpenAI 兼容模式配置自动读取 (2025-12-18)
